...
const batchStockListQuerySchema = z.object({
...
const batchStockIdParamSchema = z.object({
  id: z.string().regex(objectIdRegex, 'Invalid id format'),
});
const manualBatchStockCreateSchema = z.object({
...
const manualBatchStockUpdateSchema = z.object({
  medicine_id: z.string().regex(objectIdRegex, 'Invalid medicine_id format'),
  pack: z.string().trim().min(1),
  batch_no: z.string().trim().min(1),
  expiry_date: z.coerce.date(),
...
module.exports = { batchStockListQuerySchema, batchStockIdParamSchema, manualBatchStockCreateSchema, manualBatchStockUpdateSchema };
